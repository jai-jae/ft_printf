...
/*   Updated: 2018/12/01 12:06:59 by jaelee           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	get_prec(const char *fmt, t_pfinfo *input)
{
	...
	{
		input->flags.prec = va_arg(intput->ap[1], int); //TODO need check
		intput->i += 2;
	}
...
void	get_width(const char *fmt, t_pfinfo *input)
{
	...
	input->flags.width = va_arg(intput->ap[1], int);
	intput->i++;
}
...
void	get_flags(const char *fmt, t_pfinfo *input)
{
	...
	while (ft_strchr(FLAGS, fmt[input->i]))
	{
		...
		intput->flags.plus = fmt[input->i] == '+' ? 1 : 0;
		intput->flags.space = fmt[input->i] == ' ' ? 1 : 0;
		fmt[input->i] == '*' ? get_width(intput) : 0;
		if (fmt[input->i] = '.')
			process_prec(fmt, input);
		else if (fmt[input->i] >= '1' && fmt[input->i] <= '9')
		{
			input->width = ft_atoi(fmt + input->i);
