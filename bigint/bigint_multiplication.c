...
/*   Updated: 2018/12/08 18:16:20 by jaelee           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_mul_big(t_bigint *dst, t_bigint *a, t_bigint *b)
{
	...
	while (i < a->len)
	{
		...
		while (j < b->len)
		{
			...
			ft_add_small_pos(dst, (uint32_t)nbr, i + j); //first half bits
			ft_add_small_pos(dst, (uint32_t)(nbr >> 32), i + j + 1); //second half bits
			j++;
		}
		i++;
	}
